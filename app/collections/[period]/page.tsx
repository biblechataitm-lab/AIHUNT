...
const PERIOD_CONFIG: Record<string, { title: string; subtitle: string; since: string }> = {
  'this-week': {
    title: "This Week's Top AI Products",
    subtitle: 'The best AI tools and models launched in the past 7 days',
    since: 'week',
  },
  'this-month': {
    title: "This Month's Top AI Products",
    subtitle: 'The standout AI software and agentic tools from the past 30 days',
    since: 'month',
  },
};
...
export async function generateMetadata({ params }: CollectionPageProps): Promise<Metadata> {
  ...
  if (!config) return { title: 'Collection Not Found — AIHunt' };

  return {
    title: `${config.title} — AIHunt`,
...
async function CollectionFeed({ since }: { since: string }) {
  ...
  return (
    <ProductListWithPagination
      ...
      fetchOptions={{ since, sort: 'top' }}
      emptyTitle="No Launches in this Period"
      emptyDescription="No AI products were recorded in this collection timeframe yet."
    />
  );
}
